...
async function generateFacultyId() {
  ...
  const numbers = data.Items
    .map(i => parseInt(i.FacultyID?.replace("F", "")))
    .filter(Boolean);
...
app.get("/api/admin/faculty", async (req, res) => {
  try {
    const params = { TableName: FACULTY_TABLE };
    const data = await dynamo.scan(params).promise();

    console.log("[GET FACULTY] Count:", data.Items.length);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Upload photo to S3
      if (req.file) {
        const key = `faculty/${facultyId}.jpg`;
        ...
        photoUrl = `https://${BUCKET}.s3.ap-south-1.amazonaws.com/${key}`;
      }

      const item = {
        FacultyID: facultyId,          // PK
        Department,                   // SK
        Name,
        Designation,
        Qualification,
      ...
      console.log("[ADD FACULTY]", facultyId);
...
app.delete(
  ...
  async (req, res) => {
    ...
    console.log("[DELETE FACULTY]", facultyId, department);
