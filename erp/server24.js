...
app.get("/api/circulars", async (req, res) => {
  console.log("[GET /api/circulars] Query:", req.query);

  try {
    const { facultyId } = req.query;

    if (!facultyId) {
      console.warn("[GET CIRCULARS] facultyId missing");
      return res.status(400).json({ message: "facultyId is required" });
    }

    const params = {
      TableName: "Circulars",
      FilterExpression: "FacultyID = :fid AND IsActive = :active",
      ExpressionAttributeValues: {
        ":fid": facultyId,
        ":active": true
      }
    ...
    console.log("[GET CIRCULARS] Dynamo Params:", params);

    const data = await dynamo.scan(params).promise();

    console.log("[GET CIRCULARS] Items Found:", data.Items?.length || 0);

    res.json(data.Items || []);
  } catch (err) {
    console.error("[GET CIRCULARS ERROR]", err);
    res.status(500).json({ message: "Failed to fetch circulars" });
  }
});

app.get("/api/student/circulars", async (req, res) => {
  console.log("[GET /api/student/circulars] Query:", req.query);

  try {
    const { courseId } = req.query;

    if (!courseId) {
      console.warn("[STUDENT CIRCULARS] courseId missing");
      return res.status(400).json({ message: "courseId is required" });
    }

    const params = {
      TableName: "Circulars",
      FilterExpression: "CourseID = :cid AND IsActive = :active",
      ExpressionAttributeValues: {
        ":cid": courseId,
        ":active": true
      }
    ...
    console.log("[STUDENT CIRCULARS] Dynamo Params:", params);

    const data = await dynamo.scan(params).promise();

    console.log("[STUDENT CIRCULARS] Items Found:", data.Items?.length || 0);

    res.json(data.Items || []);
  } catch (err) {
    console.error("[GET STUDENT CIRCULARS ERROR]", err);
    res.status(500).json({ message: "Failed to fetch student circulars" });
  }
});
...
app.post(
  ...
  async (req, res) => {
    console.log("[CREATE CIRCULAR] Body:", req.body);
    console.log("[CREATE CIRCULAR] File:", req.file?.originalname || "No file");
    ...
    try {
      ...
      if (!FacultyID || !FacultyName) {
        console.warn("[CREATE CIRCULAR] Faculty data missing");
        return res.status(400).json({ message: "Faculty details missing" });
      }
      ...
      const scanParams = {
        TableName: "Circulars",
        FilterExpression: "begins_with(CircularID, :cid)",
        ExpressionAttributeValues: {
          ":cid": `CIRC_${today}`
        }
      };

      const existing = await dynamo.scan(scanParams).promise();
      const count = (existing.Items?.length || 0) + 1;
      const circularId = `CIRC_${today}_${String(count).padStart(3, "0")}`;

      console.log("[CREATE CIRCULAR] Generated CircularID:", circularId);
      ...
      if (req.file) {
        ...
        attachments.push({
          ...
          PublicUrl: uploadRes.Location
        });

        hasAttachment = true;
        console.log("[UPLOAD SUCCESS]", uploadRes.Location);
      }
      ...
      console.log("[DYNAMO PUT] Item:", item);
      ...
      console.log("[CREATE CIRCULAR] Saved successfully");
...
app.delete("/api/circulars/:circularId", async (req, res) => {
  ...
  try {
    ...
    const scanResult = await dynamo.scan({
      TableName: "Circulars",
      FilterExpression: "CircularID = :cid",
      ExpressionAttributeValues: {
        ":cid": circularId
      }
    }).promise();
    ...
    // 2️⃣ Update using REAL PRIMARY KEY
    await dynamo.update({
      TableName: "Circulars",
      Key: {
    ...
    console.log("[DELETE CIRCULAR] Soft deleted:", circularId);
...
app.get("/api/resources/faculty", async (req, res) => {
  const { facultyId } = req.query;
...
app.post("/api/resources", upload.single("file"), async (req, res) => {
  try {
    ...
    const ext = req.file.originalname.split(".").pop();
    const s3Key = `resources/${FacultyID}_${resourceId}.${ext}`;
    ...
    const item = {
      ...
      PublicURL: `https://erp-s101.s3.ap-south-1.amazonaws.com/${s3Key}`,
      S3Bucket: "erp-s101",
      S3Key: s3Key,
...
app.delete("/api/resources/:resourceId/:courseId", async (req, res) => {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      /* ---------- STEP 1: Generate FacultyID ---------- */
      const scanParams = {
        TableName: "Faculty",
        ProjectionExpression: "FacultyID",
      };

      const scanRes = await dynamo.scan(scanParams).promise();

      const existingIds = scanRes.Items.map((i) =>
        Number(i.FacultyID.replace("F", ""))
      );

      const nextId = existingIds.length
        ? Math.max(...existingIds) + 1
        : 101;

      const FacultyID = `F${nextId}`;

      console.log("[CREATE FACULTY] Generated FacultyID:", FacultyID);
      ...
      if (req.file) {
        ...
        console.log("[CREATE FACULTY] Photo uploaded:", photoUrl);
      }
      ...
      console.log("[CREATE FACULTY] Dynamo item:", item);
      ...
      console.log("[CREATE FACULTY] Faculty saved successfully");
...
app.delete(
  ...
  async (req, res) => {
    ...
    console.log("[DELETE FACULTY] FacultyID:", facultyId);
    console.log("[DELETE FACULTY] Department:", department);
    ...
    try {
      console.log("[DELETE FACULTY] Dynamo params:", params);

      await dynamo.delete(params).promise();

      console.log("[DELETE FACULTY] Deleted successfully");
